...
DB_DEST_USER=`cat ${DataSOURCE_FILE} |grep -Ewi "^[[:space:]]*his_database_username" |head -1 |cut -d = -f 2- |sed 's/^[[:space:]]*//g' |sed 's/[[:space:]]*$//g'`
DB_DEST_TNS=`cat ${DataSOURCE_FILE} |grep -Ewi "^[[:space:]]*his_database_tnsname" |head -1 |cut -d = -f 2- |sed 's/^[[:space:]]*//g' |sed 's/[[:space:]]*$//g'`
...
{ 
	...
	[[ $# -ne 1 ]] && printLog "ERROR" "dbClean.sh: Input parameters numbers must be one, ${funcName} dbtnsname: $1" && exit 1 
	typeset tnsname="$1" 
	typeset result=$(sqlplus /@"${tnsname}" <<EOF 
	set echo on 
	set feedback off 
	set heading off exit; 
EOF
) 
...
function execMigrationScript
{
    ...
    #clean data
    printLog "Info" "Begin delete user:${DB_SOURCE_USER}'s data..."
    ...
    sqlplus "/@${DB_SOURCE_TNS}" >/dev/null 2>&1 <<EOF
        set echo on
		spool ${EXECUTE_SQL_LOGS_DIR}/${sql_file}.log
        @${EXECUTE_SQL_DIR}/${sql_file}
		spool off;
        exit;
EOF
